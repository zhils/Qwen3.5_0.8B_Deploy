#pragma once

#define OPTIMIZED_BATCH_SIZE 128

#define GEMM_BLOCK_M 256
#define GEMM_BLOCK_N 256
#define GEMM_BLOCK_K 64

#define ATTENTION_BLOCK_SIZE 512
#define MLP_BLOCK_SIZE 1024

#define PREFILL_WARP_COUNT 32
#define DECODE_WARP_COUNT 16

#define SHARED_MEMORY_SIZE 131072

#define USE_BATCH_SPECIFIC_KERNELS 1
#define UNROLL_FACTOR 16

namespace qwen {
namespace config {
    constexpr int BATCH_SIZE = OPTIMIZED_BATCH_SIZE;
    constexpr int THREADS_PER_BLOCK = 1024;
    constexpr int WARPS_PER_BLOCK = THREADS_PER_BLOCK / 32;
    constexpr int MAX_SHARED_MEM = SHARED_MEMORY_SIZE;
}
}
