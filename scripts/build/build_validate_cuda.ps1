...
Write-Host "=== Compiling CUDA Validation ===" -ForegroundColor Cyan

& "C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.9\bin\nvcc.exe" `
    -x cu -c -allow-unsupported-compiler -use_fast_math -O3 `
    -I"cuda_implementation\include" `
    -o build\validate_cuda_cuda.obj cuda_implementation\validate_cuda.cu
...
$linkCmd = "`"$linker`" /NOLOGO /OUT:`"build\validate_cuda.exe`" /STACK:268435456 `"" +
           "`"build\validate_cuda_cuda.obj`" `"" +
           "`"cuda_implementation\kernels\full_attention_cuda.obj`" `"" +
           "`"cuda_implementation\kernels\linear_attention_cuda.obj`" `"" +
           "`"`"C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.9\lib\x64\cudart.lib`"`" /SUBSYSTEM:CONSOLE"

& $linker /NOLOGO /OUT:"build\validate_cuda.exe" /STACK:268435456 `
    "build\validate_cuda_cuda.obj" `
    "cuda_implementation\kernels\full_attention_cuda.obj" `
    "cuda_implementation\kernels\linear_attention_cuda.obj" `
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "`nBuild successful! Output: build\validate_cuda.exe" -ForegroundColor Green
} else {
