...
void CudaEngine::capture_decode_graph(const float* d_input, float* d_output, int position) {
    release_graph();

    forward(d_input, d_output, position);
    CHECK_CUDA(cudaDeviceSynchronize());

    CHECK_CUDA(cudaStreamBeginCapture(0, cudaStreamCaptureModeRelaxed));

    CHECK_CUDA(cudaMemcpyAsync(d_input_buf_, d_input, hidden_size_ * sizeof(float),
    ...
    CHECK_CUDA(cudaStreamEndCapture(0, &decode_graph_));

    if (!decode_graph_) {
        fprintf(stderr, "CUDA Graph capture returned null graph\n");
    ...
    if (inst_err != cudaSuccess) {
        fprintf(stderr, "CUDA Graph instantiate failed: %s\n", cudaGetErrorString(inst_err));
        if (decode_graph_) {
    ...
    graph_capture_position_ = position;
}

bool CudaEngine::replay_decode_graph() {
    if (!graph_captured_ || !decode_graph_exec_)
        return false;
    CHECK_CUDA(cudaGraphLaunch(decode_graph_exec_, 0));
    CHECK_CUDA(cudaStreamSynchronize(0));
    return true;
}

void CudaEngine::release_graph() {
